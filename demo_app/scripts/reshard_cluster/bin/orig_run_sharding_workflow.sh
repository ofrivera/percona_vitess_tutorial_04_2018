...
run_interactive 'vtctlclient -server ec2-34-208-78-29.us-west-2.compute.amazonaws.com:15999 ApplyVSchema -vschema "$(cat $VTTOP/examples/local/vschema.json)" cell1_keyspace'
...
run_interactive "python /home/ubuntu/src/github.com/jvaidya/vitess-tools/deployment_helper/deployment_helper.py --action generate --component vttablet --add --use-config-without-prompt"
...
for shard in $orig_shards; do
    tablet=$(vtctlclient -server ec2-34-208-78-29.us-west-2.compute.amazonaws.com:15999 ListShardTablets cell1_keyspace/$shard | head -1 | awk '{print $1}')
    run_interactive 'vtctlclient -server ec2-34-208-78-29.us-west-2.compute.amazonaws.com:15999 ExecuteFetchAsDba $tablet "SELECT count(*) FROM messages"'
done
